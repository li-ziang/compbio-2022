...
class scETM(BaseCellModel):
    ...
    def __init__(self,
        ...
        self.enable_ce_loss: bool = enable_ce_loss
        if self.enable_ce_loss:
            self.classify_mlp = nn.Sequential(
                nn.Linear(self.n_topics, 5),
                nn.ReLU(),
                nn.Linear(5, 2)
            )
    ...
    def classify(self,
        theta: torch.Tensor,
        assigned_cluster: Union[None, torch.Tensor]
    ) -> torch.Tensor:
        logits = self.classify_mlp(theta)
        loss = self.ce_crit(logits, assigned_cluster)
        return loss
    ...
    def forward(self,
        ...
        if self.enable_ce_loss:
            ce_loss = self.classify(theta, data_dict.get('assigned_cluster', None))
            loss += hyper_param_dict['ce_weight'] * ce_loss
        record = dict(loss=loss, nll=nll, kl_delta=kl_delta, ce_loss=ce_loss)
    ...
    def get_all_embeddings_and_nll(self,
        ...
        if inplace:
            ...
            # adata.obsm['embs'] = np.concatenate([adata.varm['rho'], adata.uns['alpha']])
            # print("Test")
            return result
        else:
