...
def visualization_data(exp_name):
    ...
    rna_embeddings = np.array([train_rna_loaders[0].dataset[i][0] for i in range(len(train_rna_loaders[0].dataset))])[:,
                     0, :]
    atac_embeddings = np.array([train_atac_loaders[0].dataset[i] for i in range(len(train_atac_loaders[0].dataset))])[:,
                      0, :]
    embeddings = np.concatenate((rna_embeddings, atac_embeddings))
    print(embeddings.shape)
    tsne_results = TSNE(perplexity=30, n_iter=1000).fit_transform(embeddings)
