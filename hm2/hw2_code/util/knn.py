...
def KNN(config, neighbors = 30, knn_rna_samples = 20000, stage1=False):  
    exp_name = config.exp_name  
    # read rna embeddings and predictions    
    print('[KNN] Read RNA data')
    db_name = os.path.basename(config.rna_paths[0]).split('.')[0]
    rna_embeddings = np.loadtxt(f'./output/{exp_name}/' + db_name + '_embeddings.txt')
    rna_predictions = np.loadtxt(f'./output/{exp_name}/' + db_name + '_predictions.txt')
    ...
    for i in range(1, len(config.rna_paths)):
        ...
        rna_embeddings = np.concatenate((rna_embeddings, np.loadtxt(f'./output/{exp_name}/' + db_name + '_embeddings.txt')), 0)
        rna_predictions = np.concatenate((rna_predictions, np.loadtxt(f'./output/{exp_name}/' + db_name + '_predictions.txt')), 0)
    ...
    atac_embeddings = np.loadtxt(f'./output/{exp_name}/' + db_name + '_embeddings.txt')
    atac_predictions = np.loadtxt(f'./output/{exp_name}/' + db_name + '_predictions.txt')
    ...
    for i in range(1, len(config.atac_paths)):
        ...
        em = np.loadtxt(f'./output/{exp_name}/' + db_name + '_embeddings.txt')
        pred = np.loadtxt(f'./output/{exp_name}/' + db_name + '_predictions.txt')
    ...
    neigh = KNeighborsClassifier(n_neighbors=neighbors, weights=config.knn_metrics, p=1)
    ...
    for i, db_name in enumerate(db_names):        
        np.savetxt(f'./output/{exp_name}/' + db_name + '_knn_predictions.txt', atac_predict[cnt:cnt+db_sizes[i]])
        np.savetxt(f'./output/{exp_name}/' + db_name + '_knn_probs.txt', conf_scores[cnt:cnt+db_sizes[i]])
        cnt += db_sizes[i]
    ...
    if len(config.atac_labels) == len(config.atac_paths):
        ...
        if stage1:
            wandb.log({'stage1 knn accuracy': correct*1./valid_sample_cnt})
        else:
            wandb.log({'final knn accuracy': correct*1./valid_sample_cnt})
