...
class TrainingProcessStage1():
    def __init__(self, config):
        ...
        if self.config.use_cuda:  
            self.model_encoder = torch.nn.DataParallel(Net_encoder(config).to(self.config.device))
            self.model_cell = torch.nn.DataParallel(Net_cell(config).to(self.config.device))
        else:
            self.model_encoder = Net_encoder(config).to(self.config.device)
            self.model_cell = Net_cell(config).to(self.config.device)
                
        # initialize criterion (loss)
        self.criterion_cell = CellLoss()
        self.criterion_encoding = EncodingLoss(dim=64, p=config.p, use_gpu = self.config.use_cuda, config=config)
        self.l1_regular = L1regularization()
        
        # initialize optimizer (sgd/momemtum/weight decay)
        if config.optim_adam:
            self.optimizer_encoder = optim.Adam(self.model_encoder.parameters(), weight_decay=1e-4, lr=5e-5)
            self.optimizer_cell = optim.Adam(self.model_cell.parameters(), weight_decay=1e-4, lr=5e-5)
        else:
    ...
    def train(self, epoch):
        ...
        for batch_idx in range(self.training_iters):
            # rna forward
            step = self.training_iters * epoch + batch_idx
            rna_embeddings = []
            ...
            for iter_rna_loader in iter_rna_loaders:
                ...
                # model forward
                rna_embedding = self.model_encoder(rna_data, True)
            ...
            for iter_atac_loader in iter_atac_loaders:
                ...
                # model forward
                atac_embedding = self.model_encoder(atac_data, False)
            ...
            encoding_loss = self.criterion_encoding(atac_embeddings, rna_embeddings, True) * self.config.reduction_loss
            ...
            wandb.log({'ce_loss': cell_loss})
            if self.config.space_alignment:
                wandb.log({'s_max': self.model_encoder.module.log_scale.max().exp()})
                wandb.log({'s_mean': self.model_encoder.module.log_scale.exp().mean()})
                wandb.log({'s_min': self.model_encoder.module.log_scale.min().exp()})
                wandb.log({'t_max': self.model_encoder.module.trans.max()})
                wandb.log({'t_mean': self.model_encoder.module.trans.mean()})
                wandb.log({'t_min': self.model_encoder.module.trans.min()})
    ...
    def write_embeddings(self, exp_name):
        ...
        for i, rna_loader in enumerate(self.test_rna_loaders):
            ...
            fp_em = open(f'./output/{exp_name}/' + db_name + '_embeddings.txt', 'w')
            fp_pre = open(f'./output/{exp_name}/' + db_name + '_predictions.txt', 'w')
