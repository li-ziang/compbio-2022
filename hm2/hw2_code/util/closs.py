...
def reduction_loss(embedding, identity_matrix, size, config=None):
    if config is not None:
        loss = config.reduction_entanglement * torch.mean(torch.abs(torch.triu(cor(embedding), diagonal=1)))
        if config.reduction_log_separation:
            loss = loss - config.reduction_separation * torch.log(torch.mean(
                torch.abs(embedding - torch.mean(embedding, dim=0).view(1, size).repeat(embedding.size()[0], 1))))
        else:
            loss = loss + config.reduction_separation * (1 / torch.mean(
                torch.abs(embedding - torch.mean(embedding, dim=0).view(1, size).repeat(embedding.size()[0], 1))))
        loss = loss + config.reduction_boundedness * torch.mean(torch.abs(embedding))
    else:
...
class EncodingLoss(nn.Module):
    def __init__(self, dim=64, p =0.8, use_gpu = True, config=None):
    ...
    def forward(self, atac_embeddings, rna_embeddings, stage1=False):
        # rna
        rna_embedding_cat = rna_embeddings[0]
        rna_reduction_loss = reduction_loss(rna_embeddings[0], self.identity_matrix, self.dim, self.config)
        ...
        # atac
        atac_reduction_loss = reduction_loss(atac_embeddings[0], self.identity_matrix, self.dim, self.config)
        ...
        # symmetry
        top_k_sim = torch.topk(
            torch.max(cosine_sim(atac_embeddings[0], rna_embedding_cat), dim=0)[0],
            int(rna_embeddings[0].shape[0] * self.p))
        rna_sim_loss = torch.mean(top_k_sim[0])
        
        sim_loss = sim_loss/len(atac_embeddings)
        
        if self.config is not None:
            loss = self.config.rna_reduction_loss * rna_reduction_loss + \
            self.config.atac_reduction_loss * atac_reduction_loss - \
            self.config.sim_loss * sim_loss - \
            self.config.rna_sim_loss * rna_sim_loss
        else:
            loss = rna_reduction_loss + atac_reduction_loss - sim_loss
        if stage1:
            wandb.log({'rna_reduction_loss': rna_reduction_loss})
            wandb.log({'atac_reduction_loss': atac_reduction_loss})
            wandb.log({'atac_sim_loss': -sim_loss})
            wandb.log({'rna_sim_loss': -rna_sim_loss})
        return loss
